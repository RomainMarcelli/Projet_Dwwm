...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['age'] > 0) {
        $uploadDir = 'uploads/';
    ...
    if (empty($errors)) {
        $stmt = $conn->prepare("INSERT INTO models (espece_id, title, description, age_id, photo_path) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->execute([$espece_id, $title, $description, $age_id, $photoPath]);
...
$especes = $conn->query("SELECT * FROM espece")->fetchAll();
$ages = $conn->query("SELECT * FROM ages")->fetchAll();
$modelsQuery = $conn->query("SELECT models.*, espece.name as espece_name, ages.age_name as age_name FROM models JOIN espece ON models.espece_id = espece.id JOIN ages ON models.age_id = ages.id");
        ...
        <table border="1">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><?= $model['espece_name'] ?></td>
                    <td><?= $model['title'] ?></td>
                    <td><?= $model['description'] ?></td>
                    <td><?= $model['age_name'] ?></td>
                    <td>
